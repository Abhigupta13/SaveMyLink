...
import { getTasksWorkspace, getMyOpenTasks, createTask, toggleTask, deleteTask, updateTask } from '@/actions/task';
import { createProject, deleteProject, renameProject } from '@/actions/project';
import { AppCacheNs, invalidateProjectsCache, invalidateTasksCache } from '@/lib/appDataCache';
import { setCachedOpenTasks } from '@/lib/openTasksCache';
...
export default function TasksClient() {
  ...
  const applyOpenTasks = useCallback((openTasks: any[], fallback: ReminderChoice | null) => {
    setReminderDefault(fallback);
    setRemind(prev => prev ?? fallback);
    reconcile(openTasks, fallback);
    setCachedOpenTasks(privateSafe, { tasks: openTasks, reminderDefault: fallback });
  ...
  const applyWorkspace = useCallback((data: {
    ...
    tasks: any[];
    openTasks: any[];
    reminderDefault: ReminderChoice | null | undefined;
  }) => {
    setProjects(data.projects || []);
    setTasks(data.tasks || []);
    applyOpenTasks(data.openTasks || [], (data.reminderDefault as ReminderChoice) || null);
  }, [applyOpenTasks]);

  const loadWorkspace = useCallback(async (projectId?: string, force?: boolean) => {
    const key = `${projectId || 'personal'}:${privateSafe ? '1' : '0'}`;
    await runStaleQuery({
      namespace: AppCacheNs.tasksWorkspace,
      cacheKey: key,
      force,
      ui: { setLoading, setRefreshing, setFailed },
      fetch: async () => {
        const res = await getTasksWorkspace(projectId);
        if (!res.success || !res.tasks) throw new Error(res.error || 'Failed');
        return {
          projects: res.projects || [],
          tasks: res.tasks || [],
          openTasks: res.openTasks || [],
          reminderDefault: res.reminderDefault,
        };
      },
      apply: applyWorkspace,
    });
  }, [privateSafe, applyWorkspace]);

  const reloadTasks = useCallback((projectId?: string) => {
  ...
  const refreshReminders = useCallback(async () => {
    ...
    applyOpenTasks(res.tasks || [], (res.reminderDefault as ReminderChoice) || null);
  }, [applyOpenTasks]);

  useEffect(() => {
    if (status !== 'authenticated') return;
    loadWorkspace(activeProject?._id);
  }, [status, privateSafe, activeProject?._id, loadWorkspace]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setDue(''); reloadTasks(activeProject?._id); refreshReminders();
    }
  ...
  return (
    <div className="container" style={{ padding: '24px 16px 120px' }}>
      ...
      <div style={{ marginBottom: '16px' }}>
        <ProjectPicker
          ...
          onCreate={async (name) => {
            ...
            if (res.success) { invalidateProjectsCache(); await reloadTasks(); switchProject(res.project); }
            else toast(res.error || 'Something went wrong', 'error');
            return res;
          }}
          onRename={async (proj, name) => {
            const res = await renameProject(proj._id, name);
            if (res.success) { invalidateProjectsCache(); await reloadTasks(activeProject?._id); if (activeProject?._id === proj._id) setActiveProject({ ...activeProject, name }); }
            else toast(res.error || 'Something went wrong', 'error');
            return res;
          }}
          onDelete={async (proj) => {
            const res = await deleteProject(proj._id);
            if (res.success) { invalidateProjectsCache(); await reloadTasks(activeProject?._id); if (activeProject?._id === proj._id) switchProject(null); }
            else toast(res.error || 'Something went wrong', 'error');
            return res;
          }}
        />
        ...
        <div className="modal-overlay" onClick={() => setEditing(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '460px' }}>
            ...
            <div style={{ display: 'grid', gap: '10px' }}>
              <input className="field" value={draft.title} onChange={e => setDraft(d => ({ ...d, title: e.target.value }))} placeholder="Task" aria-label="Task" />
      ...
      {loading && tasks.length === 0 ? (
        ...
        <LoadError what="your tasks" onRetry={() => reloadTasks(activeProject?._id)} />
