...
function SearchPageInner() {
  ...
  useEffect(() => {
    ...
    if (!trimmed) { setResults(null); setLoading(false); setRefreshing(false); return; }
    const cacheKey = trimmed.toLowerCase();
    const timer = setTimeout(async () => {
      lastQueryRef.current = cacheKey;
      const cached = cacheGet<any>(AppCacheNs.search, cacheKey);
      if (cached) {
      ...
      try {
        ...
        if (res.success) {
          setResults(res);
          cacheSet(AppCacheNs.search, cacheKey, res);
          setFailed(false);
        } else if (!cached) setFailed(true);
      } catch {
        if (lastQueryRef.current === cacheKey && !cached) setFailed(true);
      } finally {
  ...
  return (
    <div className="container" style={{ padding: '24px 16px 120px' }}>
      ...
      {refreshing && !loading && (
        <p style={{ fontSize: '0.72rem', color: 'var(--text-tertiary)', fontWeight: 600 }} aria-live="polite">Updating results…</p>
      )}
