...
import { AppCacheNs, invalidateProjectWorkspaceCache } from '@/lib/appDataCache';
import { runStaleQuery } from '@/lib/runStaleQuery';
import ProjectReportView, { buildProjectReportPlainText, shapeProjectReportInput } from '@/components/ProjectReportView';
import '@/styles/workspace.css';

type ProjectWorkspaceData = Awaited<ReturnType<typeof getProjectWorkspace>> & { success: true; project: NonNullable<Awaited<ReturnType<typeof getProjectWorkspace>>['project']> };
...
export default function ProjectWorkspaceClient({ projectId: projectIdProp }: { projectId: string }) {
  ...
  useEffect(() => {
    if (projectId) router.prefetch(`/projects/${projectId}/print`);
  }, [projectId, router]);
  ...
  const workspaceCacheKey = `${projectId}:${days}`;

  const applyWorkspace = useCallback((res: ProjectWorkspaceData) => {
  ...
  /** One server action; client cache keyed by group + activity window. */
  const load = useCallback(async (force?: boolean) => {
    if (force) invalidateProjectWorkspaceCache();
    await runStaleQuery({
      namespace: AppCacheNs.projectWorkspace,
      ...
      fetch: async () => {
        ...
        if (!res.success || !res.project) {
          if (res.error === 'Not found' || res.error === 'Unauthorized') setNotFound(true);
          throw new Error(res.error || 'Could not load this group');
        }
        return res as ProjectWorkspaceData;
      },
      apply: applyWorkspace,
    });
  }, [projectId, days, workspaceCacheKey, applyWorkspace]);
  ...
  const handleExportPdf = async () => {
    ...
    if (Capacitor.isNativePlatform()) {
      const text = buildProjectReportPlainText(
        shapeProjectReportInput(project, tasks, moms, notes, files, notesDraft),
      );
      try {
        const { Share } = await import('@capacitor/share');
        await Share.share({ title: `${project.name} — report`, text });
        toast('Choose Print or Save as PDF from the share menu.', 'info');
      } catch {
        toast('Could not open share menu', 'error');
      }
      return;
    }
    router.push(`/projects/${projectId}/print`);
  };
  ...
  if (loading && !project) {
    return <div style={{ display: 'flex', justifyContent: 'center', padding: '80px' }}><div className="loading-spinner"></div></div>;
  }
  if (notFound || (!project && failed)) {
    return (
      <div className="container" style={{ padding: '80px 16px', textAlign: 'center' }}>
      ...
      <div className="container wk print-hide" style={{ padding: '16px 16px 120px' }}>
          ...
          <div className="wk-menu-wrap" ref={menuRef}>
            ...
            {menuOpen && (
              <div className="wk-menu" role="menu">
                ...
                <button role="menuitem" onClick={() => { void handleExportPdf(); }}><Download size={16} /> Download as PDF</button>
        ...
        <div className={showRecorder ? '' : 'wk-hide'}>
          <MomSection project={project} projects={projects} myEmail={myEmail} memberOptions={memberOptions}
            onTasksCreated={() => { fetchTasks(); fetchEvents(); }} pendingOnly={section !== 'meetings'}
