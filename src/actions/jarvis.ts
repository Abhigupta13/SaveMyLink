...
async function gatherContext(userId: string, email: string, includePrivate: boolean) {
  ...
  for (const t of tasks as any[]) {
    lines.push(`TASK id=${track(t._id)} | ${t.title} | due=${d(t.dueAt) || 'none'} | ${t.completed ? 'done' : 'open'} | project=${pname.get(String(t.projectId)) || 'personal'} | assignee=${t.assigneeId?.email || t.assigneeEmail || '-'}`);
  }
...
2. Create things, ONLY by emitting an "actions" entry — you have no other way to change anything:
   - {"type":"create_task","title":"...","dueAt":"YYYY-MM-DDTHH:mm" (local time, optional),"projectName":"<exact project name from DATA, optional>","assigneeEmail":"<optional>"}
   - {"type":"create_note","title":"<short title, optional>","text":"the note body"}
   Emit an action whenever the user asks to add/remind/save/note something. Resolve relative times ("tomorrow 5pm", "in 2 hours") against the current time given above.

HARD RULES — breaking these is a serious failure:
- NEVER say you did something unless you emitted the matching action in this same reply. Do not write "Task added" without a create_task action.
...
${ctx.text || '(empty — the user has not saved anything yet)'}`;
    ...
    const createdTasks: { _id: string; title: string; dueAt?: string | null }[] = [];
