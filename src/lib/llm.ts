...
const FALLBACKS = ['gemini-3.7-flash', 'gemini-3.5-flash'];

/** First Gemini attempt — then fail over rather than stacking long waits. */
const GEMINI_PRIMARY_MS = 11_000;
const GEMINI_FALLBACK_MS = 7_000;
const GROQ_FETCH_MS = 14_000;
...
export async function chatJSON(messages: ChatMsg[]): Promise<ChatResult> {
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    const timeoutMs = i === 0 ? GEMINI_PRIMARY_MS : GEMINI_FALLBACK_MS;
    ...
    try {
      ...
      // Hung model — don't queue another 7–18s on a sibling; Groq is usually faster.
      useGroq = !!groqKey;
      break;
    }
